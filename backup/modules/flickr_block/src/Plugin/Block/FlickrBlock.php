...
class FlickrBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $plugin_id,
      $plugin_definition,
      $container->get('flickr_block.flickr_api'),
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form['flickr_photoset_id'] = [
      ...
      '#description' => $this->t('Flickr Photoset ID. Get the list of photos in a set.'),
  ...
  public function blockValidate($form, FormStateInterface $form_state) {
    ...
    if (!$response) {
      $form_state->setErrorByName('',
        $this->t('An error has occurred with Flickr API. No response given.'));
    }
    else {
  ...
  public function build() {
    ...
    if (!$response) {
      ...
      foreach ($photos as $photo) {
        ...
        if ($isFirst) {
          $uri_image_init = $this->flickrAPI->generatePhotoUri($photo, $this->configuration['flickr_header_size_init']);
          $imgClass = 'flickr-block-header-img';
          $linkClass = 'flickr-block-header-link';
          $isFirst = FALSE;
        }
        else {
          $uri_image_init = $this->flickrAPI->generatePhotoUri($photo, $this->configuration['flickr_size_init']);
        }

        if ($this->configuration['flickr_open_in_flickr']) {
          $uri_end = $this->flickrAPI->generatePhotoUriFlickr($photo['id'], $this->configuration['flickr_user_id']);
          $target = "_blank";
        }
        else {
          $uri_end = $this->flickrAPI->generatePhotoUri($photo, $this->configuration['flickr_size_end']);
          $target = "_self";
        }
